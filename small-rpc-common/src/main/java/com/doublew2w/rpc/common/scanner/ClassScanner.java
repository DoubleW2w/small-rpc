package com.doublew2w.rpc.common.scanner;
...
public class ClassScanner {
  ...
  private static void findAndAddClassesInPackageByFile(
      String packageName, String packagePath, final boolean recursive, List<String> classNameList) {
    // 获取此包的目录 建立一个File
    ...
    for (File file : dirfiles) {
      ...
      if (file.isDirectory()) {
        findAndAddClassesInPackageByFile(
            packageName + "." + file.getName(), file.getAbsolutePath(), recursive, classNameList);
      } else {
  ...
  private static String findAndAddClassesInPackageByJar(
    ...
    while (entries.hasMoreElements()) {
      // 获取jar里的一个实体 可以是目录/jar包里面的其他文件 如META-INF等文件
